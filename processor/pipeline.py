...
Steps:
  ...
  7. Write to ALL target columns (FR + FR-CA)
  8. Validate structural integrity
  9. Export
"""
...
class PipelineConfig:
    ...
    # Write translations to these columns (all get the same translation)
    target_columns: Optional[List[str]] = None  # e.g. ["FR", "FR-CA"]
    # Override which column to read source text from
    source_column_override: Optional[str] = None  # e.g. "default value"
    use_bom: bool = True
...
def run_pipeline(
    ...
    *memory*: optional pre-built TranslationMemory (from reference files
    loaded in a previous step). If None, a new one is created and populated
    from ref_data_file / ref_label_file.
    ...
    *progress_callback*: optional callable(step: str, progress: float)
    ...
    # ── Step 3: Resolve source column ────────────────────────────────
    locale_cols = find_locale_columns(original_df)
    diagnostics["locale_columns"] = str(len(locale_cols))

    # Source column: use override if provided, otherwise use source_lang
    if config.source_column_override and config.source_column_override in original_df.columns:
        source_col = config.source_column_override
    else:
        upper_map = {c.strip().upper(): c for c in locale_cols}
        source_col = upper_map.get(config.source_lang.upper())

    diagnostics["source_column"] = source_col or "NOT_FOUND"

    if not source_col:
        raise ValueError(
            f"Could not find source column "
            f"(override='{config.source_column_override}', lang='{config.source_lang}') "
            f"in the main file. Columns: {list(original_df.columns)}"
        )

    # ── Step 4: Resolve target columns ───────────────────────────────
    if config.target_columns:
        target_cols = [c for c in config.target_columns if c in original_df.columns]
        if not target_cols:
            raise ValueError(
                f"None of the target columns {config.target_columns} "
                f"exist in the file. Columns: {list(original_df.columns)}"
            )
    else:
        upper_map = {c.strip().upper(): c for c in locale_cols}
        tc = upper_map.get(config.target_lang.upper())
        target_cols = [tc] if tc else []

    if not target_cols:
        raise ValueError(
            f"Could not find any target columns for '{config.target_lang}' "
            f"in the file. Locale columns: {locale_cols}"
        )

    diagnostics["target_columns"] = ", ".join(target_cols)

    # ── Step 5: Load reference files & build memory ──────────────────
    ...
    for row_idx in range(total_rows):
        ...
        # Translate this cell (once — then write to all target columns)
        ct = translate_cell(
            value=source_value,
            row_index=row_idx,
            column_name=target_cols[0],
        ...
        # Write translation to ALL target columns
        for tc in target_cols:
            translated_df.at[row_idx, tc] = ct.translated
